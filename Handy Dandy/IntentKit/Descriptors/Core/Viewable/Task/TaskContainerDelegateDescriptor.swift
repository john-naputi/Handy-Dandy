...
struct TaskContainerDelegateDescriptor<TContainer: TaskContainer>: View {
    ...
    @State private var taskToEdit: ChecklistTask?
    ...
    private func makeEditableIntent(task: ChecklistTask = ChecklistTask.emptyDraft(), mode: EditMode) -> EditableTaskIntent {
...
#Preview {
    let plan = Plan(title: "Plan", description: "Description", planDate: .now, tasks: [
        ChecklistTask(title: "First", description: "First description"),
        ChecklistTask(title: "Second", description: "Second description")
    ])
